...
class AgentConfig:
    ...
    def __post_init__(self):
        """Optimize settings for GLM-4.5 models"""
        if "glm-4.5" in self.model_name.lower():
            # Apply GLM-4.5 optimizations from GLM.md
            self.max_iterations = 150  # Increased from 100 for complex workflows
            self.native_max_auto_continues = 40  # Increased from 25 for uninterrupted tool chains
            logger.info(f"GLM-4.5 detected: Using optimized limits - max_iterations={self.max_iterations}, native_max_auto_continues={self.native_max_auto_continues}")
    
    def get_dynamic_temperature(self, iteration: int) -> float:
        """Get dynamic temperature for GLM models based on iteration"""
        if "glm" in self.model_name.lower():
            # Import the function from llm.py
            from services.llm import get_dynamic_temperature
            return get_dynamic_temperature(iteration, base_temp=0.6)
        return 0  # Default for other models
...
class AgentRunner:
    ...
    def get_max_tokens(self) -> Optional[int]:
        if "sonnet" in self.config.model_name.lower():
            ...
            return 8192
        elif "glm-4.5" in self.config.model_name.lower():
